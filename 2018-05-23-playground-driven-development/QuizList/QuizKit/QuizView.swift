...
//    let correctImage = UIImage(named: "check-icon", in: bundle, compatibleWith: self.traitCollection)
    ...
    verticalStackView.snp.makeConstraints {
      $0.top.equalToSuperview().offset(self.layoutMargins.top)
      $0.leading.equalToSuperview().offset(self.layoutMargins.left)
      $0.trailing.equalToSuperview().offset(-self.layoutMargins.right)
    }

    buttonsStackView.snp.makeConstraints {
      $0.top.equalTo(verticalStackView.snp.bottom).offset(30)
      $0.bottom.equalToSuperview().offset(-self.layoutMargins.bottom)
